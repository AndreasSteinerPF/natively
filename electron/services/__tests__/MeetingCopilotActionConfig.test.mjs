...
describe('meeting-copilot action config defaults', () => {
  ...
  test('system-design deeper prompt is phase-aware and skimmable', () => {
    ...
    assert.match(prompt, /Why/);
    assert.match(prompt, /Watch For/);
    assert.match(prompt, /Return exactly these sections in this order: Verdict, Do This Live, Watch For/);
    assert.doesNotMatch(prompt, /Return exactly these sections in this order: Verdict, Fixes, Say, Stress Test/);
    assert.match(prompt, /2-5 numbered, ranked live steps/);
    assert.match(prompt, /Each step must use this exact shape with one short line per field: Say: \.\.\., Write: \.\.\., Why: \.\.\./);
    assert.match(prompt, /Do not return separate Fixes, Say, Why, or Stress Test sections/);
    assert.match(prompt, /live interview/i);
    assert.match(prompt, /skimmable/i);
  });

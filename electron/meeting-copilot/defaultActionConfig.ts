...
const SYSTEM_DESIGN_GUIDE_PROMPT = [
    ...
    'Treat Write as an exact drawing script: assume I will draw exactly what it says, in order, left-to-right and top-to-bottom.',
    ...
    'For High-Level Architecture Write lines, prefer explicit drawing commands such as `Draw box X`, `Draw arrow A -> B labeled lookup`, and `Draw separate arrow A -> C labeled miss path`.',
    'Do not use chained arrows like `A -> B -> C` unless B really calls, sends to, or depends on C. If A talks to both B and C, write two separate arrow commands.',
    'In High-Level Architecture, never imply a cache, queue, or derived store is the caller of a source-of-truth database unless that is the real runtime flow.',
    ...
    'Before advancing out of High-Level Architecture, mentally trace one write, one read, one late/corrected event, and one reprocess/admin event through the diagram.',
    'Once all major architecture slices appear complete, do not leave High-Level Architecture unless the transcript or action history shows the architecture has been reviewed or stress-tested.',
    'If any path is missing, floating, or ambiguous about ownership/order/source-of-truth, choose REPAIR rather than ADVANCE.',
...
const SYSTEM_DESIGN_DEEPER_PROMPT = [
    ...
    'Return exactly these sections in this order: Verdict, Do This Live, Watch For.',
    'Verdict: one sentence starting with VALIDATE, REPAIR, or STRESS-TEST, and state the highest-risk issue first.',
    'Do This Live must contain 2-5 numbered, ranked live steps. Each step must use this exact shape with one short line per field: Say: ..., Write: ..., Why: ...',
    'Put Say first, because that is what I read while speaking. Make Say a concise interview-ready line I can say out loud immediately.',
    'Write is the exact board action, Excalidraw edit, annotation, arrow change, or `No board change needed`.',
    'Why is the reasoning I need to defend the fix or validation: the failure mode, tradeoff, scale constraint, ordering issue, consistency issue, or interview risk.',
    'If a step uses a derived number or capacity estimate, Why must show the quick math in speakable form.',
    'Watch For should contain 1-3 short interviewer challenges, traps, or follow-up answers to be ready for.',
    'Do not return separate Fixes, Say, Why, or Stress Test sections; integrate the fix, speaking line, and rationale in each live step.',

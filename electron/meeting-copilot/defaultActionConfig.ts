...
const SYSTEM_DESIGN_GUIDE_PROMPT = [
    ...
    'If the problem statement or current board state is unclear, make one pragmatic assumption and continue, but name the assumption in Why or Watch For.',
    'For underspecified fresh prompts like "Design a notification system", Requirements & Scope should sound like a natural interview opening: clarify the core use case, success criteria, and scale before committing to details.',
    'Do not immediately enumerate every possible trigger, channel, legal constraint, feature, or large scale assumption when those were not stated.',
    'Limit fresh ambiguous scope handling to 1-2 clarifying questions plus at most one small labeled assumption, then proceed with the simplest useful scope.',
